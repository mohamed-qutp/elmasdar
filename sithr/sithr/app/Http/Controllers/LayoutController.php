...
class LayoutController extends Controller
{
    public function __construct()
    {
        ...
        if (Auth::check()) {
            ...
            $vacation_requests = Vacations_taken::where('status','=','pending')->whereColumn('back_up_user','=','accepted_by_substitute_employee')->where('user_id','!=',Auth::user()->id)->count();
            $vacation_substitute_employee_requests = Vacations_taken::where('status','=','pending')->where('user_id','!=',Auth::user()->id)->where('back_up_user','=',Auth::user()->id)->count(); 
            ...
            View::share('vacation_substitute_employee_requests', $vacation_substitute_employee_requests);
